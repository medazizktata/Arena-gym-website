...
const SubscriptionBox = ({
  ...
  const boxStyle = type === 1 ? `bg-yellowMain ${boxBaseStyle}` : `bg-blackBG text-white ${boxBaseStyle}`;
  ...
  return (
    <div className={`${boxStyle} ${sizeStyle} ${boxClass}`}>
      ...
      <div className="flex flex-col items-start justify-start mt-10">
        <div className="flex items-baseline">
          <h2 className={`text-5xl font-Urbanist ${priceColor} font-bold`}>{price}</h2>
          <span className={`text-3xl font-Urbanist ml-2.5 ${priceColor} font-bold`} style={{ position: 'relative', bottom: '0.35em' }}>
            TND
          </span>
        </div>
        <p className="mb-6">{duration}</p>
