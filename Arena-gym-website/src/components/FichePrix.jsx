...
const SubscriptionBox = ({
  ...
  const boxStyle = isSelected
    ? `${boxBaseStyle} bg-white text-black`
    : type === 1
    ? `${boxBaseStyle} bg-yellowMain text-black`
    : `${boxBaseStyle} bg-blackBG text-white`;

  const iconColor = isSelected ? "black" : type === 1 ? "blackBG" : "white";
  ...
  const boxClass = isSelected
    ? "border-2 border-black shadow-cardYellowMain"
    : type === 1
    ? "border-2 border-transparent shadow-cardSmallYellowDark hover:shadow-cardYellowMain hover:border-black"
    : "border-2 border-white shadow-cardSmallGreyLight hover:shadow-cardYellowMain hover:border-black";
  ...
  return (
    <div
      className={`${boxStyle} ${sizeStyle} ${boxClass} group hover:bg-white hover:text-black px-6 max-w-screen-sm:scale-60 max-w-screen-md:scale-90 lg:scale-100 transition duration-300 ease-in-out cursor-pointer ${isAnimating ? `border-2 ${isSelected ? "border-green-400" : ""} transition duration-300 ease-in-out` : `border-2 ${isSelected ? "border-green-400" : "border-transparent"} transition duration-300 ease-in-out`}`}
      onClick={handleCompSelection}
    >
      {type === 1 && (
        <div
          className={`text-lg font-bold font-Urbanist ${
            isSelected ? "bg-yellowMain" : "bg-white"
          } group-hover:bg-yellowMain transition duration-300 ease-in-out cursor-pointer p-1.5 w-40 text-center absolute top-5 -right-1 transform rotate-45 translate-x-1/4 -translate-y-1/10`}
        >
          Best Offer
        </div>
      )}
