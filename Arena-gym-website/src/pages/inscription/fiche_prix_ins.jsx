...
const SubscriptionBox2 = ({
  ...
  const boxStyle =
    type === 1 ? "bg-yellowMain relative" : "bg-blackBG text-white relative";
  const priceColor = type === 1 ? "text-black" : "text-yellowMain";
  const boxClass = type === 1 ? "shadow-yellow" : "shadow-white";
  const buttonStyle =
    type === 1 ? "bg-black text-white" : "bg-yellowMain text-black";
  const icon = svgIcon || (
  ...
  return (
    <div
      className={`p-4 ${boxStyle} ${boxClass} max-w-sm w-[80%] border border-2 border-transparent shadow-cardSmallYellowDark hover:border-white transition-all duration-500 hover:shadow-cardYellowLight cursor-pointer transition-all duration-500`}
